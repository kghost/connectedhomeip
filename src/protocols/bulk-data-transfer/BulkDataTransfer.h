...
/**
 *   @namespace nl::Chip::Profiles::BulkDataTransfer
 *
 *   @brief
 *     This namespace includes all interfaces within Chip for the
 *     Chip Bulk Data Transfer (BDX) profile.
 */
 ...
 * is the initiator.
 */
class DLL_EXPORT SendInit {
public:
    // constructor
    SendInit();
    // initializer
    CHIP_ERROR init(bool aSenderDrive, bool aReceiverDrive, bool asynchMode,
    ...
    // packing and parsing
    CHIP_ERROR pack(PacketBuffer*);
    uint16_t packedLength();
    static CHIP_ERROR parse(PacketBuffer*, SendInit&);
    // comparison
...
public:
    ...
    SendAccept();
    // initializer
    CHIP_ERROR init(uint8_t, uint16_t, ReferencedTLVData*);
    // packing and parsing
    CHIP_ERROR pack(PacketBuffer*);
    uint16_t packedLength();
    static CHIP_ERROR parse(PacketBuffer*, SendAccept&);
    // comparison
...
public:
    ...
    ReceiveAccept();
    // initializer
    CHIP_ERROR init(uint8_t, uint16_t, uint64_t, ReferencedTLVData*);
    CHIP_ERROR init(uint8_t, uint16_t, uint32_t, ReferencedTLVData*);
    // packing and parsing
    CHIP_ERROR pack(PacketBuffer*);
    uint16_t packedLength();
    static CHIP_ERROR parse(PacketBuffer*, ReceiveAccept&);
    // comparison
...
public:
    ...
    BlockQuery();
    // initializer
    CHIP_ERROR init(uint8_t);
    // packing and parsing
    CHIP_ERROR pack(PacketBuffer*);
    uint16_t packedLength();
    static CHIP_ERROR parse(PacketBuffer*, BlockQuery&);
    // comparison
...
public:
    ...
    BlockSend();
    // initializer
    CHIP_ERROR init(uint8_t, uint64_t, uint8_t*);
    // packing and parsing
    CHIP_ERROR pack(PacketBuffer*);
    uint16_t packedLength();
    static CHIP_ERROR parse(PacketBuffer*, BlockSend&);
    // comparison
 ...
 * passenger.
 */
class NL_DLL_EXPORT ChipBdxClient {
public:
    // constructors
    ChipBdxClient();
    // init/Shutdown
    CHIP_ERROR initClient(ChipExchangeManager*, void*, ReferencedString&, uint16_t, uint64_t, uint64_t, bool);
    CHIP_ERROR shutdownClient();
    CHIP_ERROR shutdownClient(CHIP_ERROR aErr);
    ...
    typedef void (*ErrorHandler)(void *aAppState, CHIP_ERROR anErrorCode);
    // protocol entry points
    CHIP_ERROR initBdxReceive(bool iCanDrive,
    ...
    CHIP_ERROR initBdxSend(bool iCanDrive, bool uCanDrive, bool aSynchOK,
    ...
    CHIP_ERROR awaitBdxInit(); //TBD
private:
    // the main handler for messages arriving on the exchange
    static void handleResponse(ExchangeContext*, const IPPacketInfo *, const ChipMessageInfo*, uint32_t, uint8_t, PacketBuffer*);
    // handler for unexpected connection close indications
    static void handleConnectionClosed(ExchangeContext *anExchangeCtx, ChipConnection *con, CHIP_ERROR conErr);
public:
    ...
    void DispatchErrorHandler(CHIP_ERROR anErrorCode);
...
public:
    ...
    uint64_t theLength;
    uint8_t theBlockCounter;
    const ChipFabricState *theFabricState;  // [READ ONLY] Fabric state object
    ChipExchangeManager *theExchangeMgr;    // [READ ONLY] Exchange manager object
    ChipConnection *theConnection;          // Connection to use to talk to the BDX server; NULL means use UDP.
