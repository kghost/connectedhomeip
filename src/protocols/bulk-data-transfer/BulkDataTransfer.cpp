...
#include <protocols/bulk-data-transfer/BulkDataTransfer.h>
...
/*
 * here are the method definitions for the ChipBdxClient class
 *
...
ChipBdxClient::ChipBdxClient()
{
  ...
  theExchangeMgr = NULL;
  theConnection = NULL;
  theEncryptionType = kChipEncryptionType_None;
  theKeyId = ChipKeyId::kNone;
...
/*
 * parameters:
 * - ChipExchangeManager &anExchangeMgr, an exhcnage manager to use
 ...
 * - bool aWideRange,
 return: error/status
 */
WEAVE_ERROR ChipBdxClient::initClient(ChipExchangeManager *anExchangeMgr,
                                       void *aAppState,
                                       ReferencedString &aFileDesignator,
                                       uint16_t aMaxBlockSize,
                                       uint64_t aStartOffset,
 ...
 * return: error/status
 */
WEAVE_ERROR ChipBdxClient::shutdownClient()
{
 ...
 * return: error/status
 */
WEAVE_ERROR ChipBdxClient::shutdownClient(WEAVE_ERROR aErr)
{
 ...
 * return: error/status
 */
WEAVE_ERROR ChipBdxClient::initBdxReceive(bool iCanDrive,
                                           ...
                                           XferDoneHandler aDoneHandler, ErrorHandler anErrorHandler)
{
  ...
  err=theExchangeCtx->SendMessage(kChipProfile_BDX, kMsgType_ReceiveInit, buffer);
  buffer = NULL;
 ...
 * return: error/status
 */
WEAVE_ERROR ChipBdxClient::initBdxSend(bool iCanDrive, bool uCanDrive, bool aSynchOK,
...
// a single node at any one time.
  ...
  err=theExchangeCtx->SendMessage(kChipProfile_BDX, kMsgType_SendInit, buffer);
  buffer = NULL;
  if (err==WEAVE_NO_ERROR) return err;
cleanup:
  DispatchErrorHandler(err);
...
/*
 *
 WEAVE_ERROR ChipBdxClient::awaitBdxInit(uint64_t) {} //TBD
*/
/*
 *
WEAVE_ERROR ChipBdxClient::awaitBdxInit(uint64_t, IPAddress) {} //TBD
*/
/*
 * here's a utility function for sending a block query
 * parameter: ChipBdxClient bdxApp,
 * return: error/status
 */
WEAVE_ERROR sendBlockQuery(ChipBdxClient *bdxApp)
{
  ...
  TRY(bdxApp->theExchangeCtx->SendMessage(kChipProfile_BDX, kMsgType_BlockQuery, outBuffer));
  outBuffer = NULL;
  return WEAVE_NO_ERROR;
}
/*
 ...
 * return: error/status
 */
WEAVE_ERROR sendBlockEOFAck(ChipBdxClient *bdxApp)
{
  PacketBuffer *outBuffer=PacketBuffer::New();
  if (outBuffer==NULL) return WEAVE_ERROR_NO_MEMORY;
  BlockEOFAck outMsg;
  TRY(outMsg.init(bdxApp->theBlockCounter));
  TRY(outMsg.pack(outBuffer));
  TRY(bdxApp->theExchangeCtx->SendMessage(kChipProfile_BDX, kMsgType_BlockEOFAck, outBuffer));
  outBuffer = NULL;
  return WEAVE_NO_ERROR;
}
/*
 ...
 * return: error/status
 */
WEAVE_ERROR sendBlock(ChipBdxClient *bdxApp)
{
  uint64_t length;
  uint8_t *data = NULL;
  bool isLast;
  ...
  TRY(bdxApp->theExchangeCtx->SendMessage(kChipProfile_BDX, msgType, outBuffer));
 ...
 * - ChipMessageInfo *aChipMsgInfo, the message information for the message
 ...
 */
void ChipBdxClient::handleResponse(ExchangeContext *anExchangeCtx, const IPPacketInfo *pktInfo, const ChipMessageInfo *aChipMsgInfo,
                                    uint32_t aProfileId, uint8_t aMessageType, PacketBuffer *aPacketBuffer)
{
  ...
  ChipBdxClient *bdxApp = (ChipBdxClient *)anExchangeCtx->AppState;
  // Verify that the message is a BDX message and get out if it's not
  if (aProfileId!=kChipProfile_BDX)
...
void ChipBdxClient::handleConnectionClosed(ExchangeContext *anExchangeCtx, ChipConnection *con, WEAVE_ERROR conErr)
{
  ChipBdxClient *bdxClient = (ChipBdxClient *)anExchangeCtx->AppState;
...
void ChipBdxClient::DispatchReceiveAccept(ReceiveAccept *aReceiveAcceptMsg)
{
...
void ChipBdxClient::DispatchSendAccept(SendAccept *aSendAcceptMsg)
{
...
void ChipBdxClient::DispatchRejectHandler(StatusReport *aReport)
{
  if (theRejectHandler)
  {
...
void ChipBdxClient::DispatchPutBlockHandler(uint64_t length,
                                             uint8_t *dataBlock,
                                             bool isLastBlock)
{
...
void ChipBdxClient::DispatchGetBlockHandler(uint64_t *pLength,
                                             uint8_t **aDataBlock,
                                             bool *isLastBlock)
{
...
void ChipBdxClient::DispatchXferErrorHandler(StatusReport *aXferError)
{
...
void ChipBdxClient::DispatchXferDoneHandler()
{
...
void ChipBdxClient::DispatchErrorHandler(WEAVE_ERROR anErrorCode)
{
  if (theErrorHandler)
  {
